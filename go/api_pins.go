...
// toStatus - convert string array to status array
...
func (c *PinsApiController) PinsGet(w http.ResponseWriter, r *http.Request) {
	...
	name := query.Get("name")
	match := TextMatchingStrategy(query.Get("match"))
	status := toStatus(strings.Split(query.Get("status"), ","))
	before, _ := time.Parse(time.RFC3339, query.Get("before"))
	...
	//meta := query["meta"]
	json.Unmarshal([]byte(query.Get("meta")), &meta)
